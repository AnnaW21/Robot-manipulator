...
void Go() {
  if (executive_flag) {
    ...
    myservo.write(int(traj_pos[maxPackageExecutiveIndex][6]));
  }
}
...
void Read(const Text &COM_port, float step_pos[7], double step_speed[6], bool withServo) {
  for (int i = 0; i < 6; i++) {
    step_pos[i] = COM_port.getSub(i, '/').toFloat();
    step_speed[i] = COM_port.getSub(i + 6, '/').toFloat();
  }
  if (withServo) {
    step_pos[6] = COM_port.getSub(12, '/').toFloat();
  }
}
...
void InputData() {
  if (Serial.available() > 0) {
    ...
    if (COM_port == "stop") {
      ...
      Read(COM_port, traj_pos[package_read_index], traj_speed[package_read_index], true);
...
void OutputData() {
  if (stepper1.getCurrentDeg() * dir[0] > traj_pos[package_executive_index[0]][0] * dir[0]) {
    ...
    if(package_executive_index[0] > maxPackageExecutiveIndex){
      maxPackageExecutiveIndex = package_executive_index[0];
    }
  }
  ...
  if(maxPackageExecutiveIndex < maxArrayElement(package_executive_index)){
    maxPackageExecutiveIndex = maxArrayElement(package_executive_index);
  }
